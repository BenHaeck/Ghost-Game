...
impl <TTile> WallMan<TTile> where TTile:Clone{
	...
	pub fn assign_tile_types <F>(&mut self, f:F, check_dist: Vec2, coll_dim: Vec2) where F: Fn(IVec2, BVec2)-> TTile {
		for i in 0..self.wall_sprites.len() {
			...
			for j in [-1.0, 1.0].iter() {
				...
				if MultiCollider::check_intersection_slice(self.wall_colliders.as_slice(), &collbox!(check_pos, coll_dim)) {
